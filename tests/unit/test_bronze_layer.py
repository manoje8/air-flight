...
class TestBronzeIngestion:
    ...
    def test_creates_json_file_and_pushes_xcom(
        ...
        expected_path = path_redir(
            "/opt/airflow/data/bronze/flight_2025-12-10-14-00-00.json"
        )
        ...
        ), f"Bronze JSON file should be created at {expected_path}"
    ...
    def test_skip_on_empty_states(
        self, tmp_path, mock_opensky_empty_response, mock_context, path_redir
    ):
    ...
    def test_json_content_matches_api_response(
        self, tmp_path, mock_opensky_response, mock_context, path_redir
    ):
